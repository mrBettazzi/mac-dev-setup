...
This document comes from a very useful articole, published on [GitHub](https://github.com) by [Nicolas Hery](https://github.com/nicolashery).
It describes how to set up a developer environment on a new MacBook or iMac. 
...
The document assumes you are new to Mac. The steps below were tested on **OS X High Sierra 10.13**.
...
Should you need to boot your iMac in Recovery Mode, keep pressed *Command-R* during the startup.
...
**WARN**: in OSX High Sierra you should enable R/W on the brew directory. Boot in RECOVERY MODE (Command + R), then open a Terminal Session and :
...
Downloaded packages are copied in `$/usr/local/Cellar`. 
Installed packages are put in `$/usr/local/lib`.
Links can be found in `$/usr/local/bin` (visible to $PATH). 
...
As you can see in the ouput from Homebrew, before we can use MySQL we first need to set it up with:
    ...
    $ unset TMPDIR
    $ mkdir /usr/local/var
    $ ### older : ### mysql_install_db --verbose --user=`whoami` --basedir="$(brew --prefix mysql)" --datadir=/usr/local/var/mysql --tmpdir=/tmp
...
If the `/usr/bin` directory is not already in the $PATH we need to tell npm where to find the Xcode Command Line Tools, by running:
...
Global node modules are put in `$/usr/bin` and `$/usr/local/lib`. 
...
Keep the default file name and skip the passphrase by just hitting Enter both times. 
The key is saved in `~/.ssh/id_rsa.pub`.

Add the key to your Heroku account:
